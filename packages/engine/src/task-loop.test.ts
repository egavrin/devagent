...
describe("TaskLoop", () => {
  ...
  describe("plan-aware continuation", () => {
    ...
    it("continues when no plan exists but session is early (low iterations)", async () => {
      // Simulates: LLM does 2 tool calls, then produces text-only "here's what I'll do"
      // without creating a plan — should continue since we're early in the session
      const provider = createMockProvider([
        // Iteration 1: tool call (productive work)
        [
          {
            type: "tool_call",
        ...
        // Iteration 2: text-only "progress update" without plan
        [
          { type: "text", content: "I found an issue. I'll continue reviewing the rest of the files." },
          { type: "done", content: "" },
        ],
        // Iteration 3: tool call (continues after nudge)
        [
          {
            type: "tool_call",
    ...
    it("records git_diff path targets as modified files for review continuity", async () => {
      const provider = createMockProvider([
        [
          {
            type: "tool_call",
            content: '{"path":"packages/core/src/config.ts","staged":true}',
      ...
      expect(ss.getModifiedFiles()).toEqual(["packages/core/src/config.ts"]);
      expect(ss.getReadonlyCoverage().get("git_diff")).toEqual(["packages/core/src/config.ts"]);
    });
...
+    new line 2`;
  ...
  describe("file-read deduplication", () => {
    ...
    it("nudges finalization when readonly calls stop adding new coverage", async () => {
      const responses: StreamChunk[][] = [];
      for (let i = 0; i < 7; i++) {
        responses.push([
          {
      ...
      const result = await loop.run("Read repeatedly");
      const nudge = result.messages.find(
        (m) =>
          m.role === MessageRole.SYSTEM
          && m.content?.includes("Readonly inspections are no longer increasing coverage or findings"),
      );
      expect(nudge).toBeDefined();
      const blocked = result.messages.find(
        (m) =>
          m.role === MessageRole.TOOL
          && m.content?.includes("Readonly inspection paused due to repeated no-progress cycles"),
      );
      expect(blocked).toBeDefined();
    });

    it("does not reset readonly no-progress tracking when update_plan is interleaved", async () => {
      const responses: StreamChunk[][] = [];
      const repeatingPlan = JSON.stringify([
        { description: "Collect scoped diffs", status: "in_progress" },
        { description: "Assess risks/regressions", status: "pending" },
        { description: "Report prioritized findings", status: "pending" },
      ]);
      ...
      for (let i = 0; i < 7; i++) {
        responses.push([
          {
            type: "tool_call",
            content: JSON.stringify({ steps: repeatingPlan }),
            toolCallId: `call_plan_${i}`,
            toolName: "update_plan",
          },
          {
            type: "tool_call",
      ...
      const result = await loop.run("Review repeatedly");
      const nudge = result.messages.find(
        (m) =>
          m.role === MessageRole.SYSTEM
          && m.content?.includes("Readonly inspections are no longer increasing coverage or findings"),
...
type RunResult = string
  ...
  describe("improved formatToolSummary — richer summaries", () => {
      ...
      const searchTool: ToolSpec = {
        ...
        handler: async () => ({
          ...
          output: [
            ...
            "",
            "packages/engine/src/plan-tool.ts",
            "  42: export function createPlanTool(ctx: Context) {",
            "",
            "packages/engine/src/task-loop.ts",
      ...
      const findTool: ToolSpec = {
        ...
        handler: async () => ({
          ...
          output: [
            ...
            "packages/engine/src/task-loop.test.ts",
            "packages/engine/src/session-state.test.ts",
          ].join("\n"),
          error: null,
          artifacts: [],
        }),
      ...
      const gitStatusTool: ToolSpec = {
        ...
        handler: async () => ({
          ...
          output: [
            ...
            " M packages/engine/src/task-loop.ts",
            "A  packages/engine/src/finding-tool.ts",
            "A  packages/engine/src/session-state.ts",
            "?? tmp/debug.log",
          ].join("\n"),
          error: null,
          artifacts: [],
    ...
    it("diagnostics summary has severity counts and diagnostic lines", async () => {
      const diagOutput = [
        "packages/engine/src/task-loop.ts:42:10 - error TS2322: Type 'string' is not assignable to type 'number'.",
        "packages/engine/src/task-loop.ts:105:5 - error TS2339: Property 'foo' does not exist on type 'Bar'.",
        "packages/engine/src/task-loop.ts:88:3 - warning TS6133: Variable 'x' is declared but never used.",
      ].join("\n");
