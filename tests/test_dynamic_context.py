...
class TestDynamicContextTracker:
    ...
    def test_multiple_updates(self, tracker):
        ...
        action2 = MagicMock()
        action2.tool = 'grep'
        action2.parameters = {'pattern': 'ClassName'}  # Use CamelCase pattern that will be recognized
    ...
    def test_should_refresh_repomap(self, tracker):
        ...
        assert tracker.should_refresh_repomap() == False

        # After 2 steps (even number), should refresh
        tracker.step_count = 2
        assert tracker.should_refresh_repomap() == True
