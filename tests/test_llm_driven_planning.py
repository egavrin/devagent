...
from ai_dev_agent.agents.work_planner import Task, Priority, TaskStatus
...
class TestQueryComplexityAssessment:
    ...
    def test_assess_simple_query_returns_direct(self):
        ...
        client = Mock()
        client.complete = Mock(return_value=json.dumps({
            "approach": "direct",
    ...
    def test_assess_complex_query_returns_complex_plan(self):
        ...
        result = _assess_query_with_llm(
            client,
            "implement user authentication with JWT, add tests, update docs"
        )
    ...
    def test_assess_handles_json_in_markdown(self):
        ...
        client = Mock()
        client.complete = Mock(return_value='''```json
{
    "approach": "direct",
    "reasoning": "Test",
    "estimated_tasks": 1,
...
class TestEarlyTerminationDetection:
    ...
    def test_satisfied_query_returns_true(self):
        ...
        remaining = [
            Task(title="Verify count", description="Double-check the count")
        ]

        result = _check_if_query_satisfied(
            client,
            "how many lines in commands.py",
            completed,
            remaining
        )
        ...
        completed = [
            {
                ...
                "result": {"final_message": "Feature implemented"}
            }
        ]
        remaining = [
...
```''')
        ...
        result = _check_if_query_satisfied(
            client,
            "test",
            [{"task_title": "Done", "result": {"final_message": "OK"}}],
            []
        )
