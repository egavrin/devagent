...
from ai_dev_agent.core.utils.context_budget import ContextBudgetConfig, prune_messages
...
class TestPruningMessageRolePreservation:
    ...
    def test_pruning_preserves_assistant_tool_calls(self):
        ...
        assistant_with_tools = [msg for msg in pruned if msg.role == "assistant" and msg.tool_calls]
...
class TestIntegrationPruningAndSanitization:
    ...
    def test_pruning_then_sanitization_removes_orphans(self):
        ...
        for tool_msg in tool_messages:
            assert (
                tool_msg.tool_call_id in assistant_tool_calls
    ...
    def test_realistic_review_scenario(self):
            ...
            messages.extend(
                [
                    Message(
                        ...
                        tool_calls=[{"id": f"call_{i}", "function": {"name": "analyze"}}],
                    ),
        ...
        for msg in sanitized:
            if msg.role == "tool":
                assert (
                    msg.tool_call_id in tool_call_ids or msg.tool_call_id == "pruned"
